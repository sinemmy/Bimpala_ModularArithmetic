...
class BilinearGatedFC(nn.Module):
    ...
    def forward(self, x):
        return self.transform(x) * self.gate(x)


class StrictlyBilinearLSTMCell(nn.Module):
    """Fully bilinear LSTM cell — no sigmoid or tanh anywhere.

    Gate projections (i, f, g, o) are all bilinear:
        bilinear([x, h]) = transform([x, h]) * gate([x, h])

    Cell-state readout is also bilinear instead of tanh(c):
        h' = o * (cell_transform(c') * cell_gate(c'))
    """
    def __init__(self, input_size, hidden_size, bias=False):
        super(StrictlyBilinearLSTMCell, self).__init__()
        self.hidden_size = hidden_size
        # Four gates packed into one bilinear projection pair
        ...
        self.cell_transform = nn.Linear(hidden_size, hidden_size, bias=bias)
        self.cell_gate      = nn.Linear(hidden_size, hidden_size, bias=bias)
    ...
    def forward(self, x, hx):
        ...
        c_new = f * c + i * g
        h_new = o * (self.cell_transform(c_new) * self.cell_gate(c_new))  # fully bilinear
        return h_new, c_new


class StrictlyBilinearLSTM(nn.Module):
    """Strictly bilinear LSTM layer wrapping StrictlyBilinearLSTMCell.
    ...
    """
    def __init__(self, input_size, hidden_size, bias=False):
        super(StrictlyBilinearLSTM, self).__init__()
        self.hidden_size = hidden_size
        self.cell = StrictlyBilinearLSTMCell(input_size, hidden_size, bias=bias)
...
class BilinearGatedActivation2D(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, padding, bias=False):
        ...
        self.transform = nn.Conv2d(in_channels, out_channels, kernel_size=kernel_size,
                                   padding=padding, bias=bias)
        self.gate      = nn.Conv2d(in_channels, out_channels, kernel_size=kernel_size,
                                   padding=padding, bias=bias)
...
class strictlyBilinearCNN(nn.Module):
    def __init__(self, obs_space, num_outputs):
        super(strictlyBilinearCNN, self).__init__()
        ...
        self.lstm     = StrictlyBilinearLSTM(input_size=512, hidden_size=256)
    ...
    def forward(self, obs, hx=None):
        ...
        x = self.gatedfc1(x)
        x = self.gatedfc2(x)
        x, hx = self.lstm(x, hx)   # strictly bilinear LSTM: (batch, 256)
